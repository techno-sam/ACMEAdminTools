...
public class ServerPlayerMixin {
    ...
    private void restrictAndSendModeratorEffects(ServerGamePacketListenerImpl instance, Packet<?> packet, Operation<Void> original) {
        boolean moderatorOnly = false;

        if (packet instanceof ClientboundUpdateMobEffectPacket mePacket && mePacket.getEffect() instanceof ModeratorOnlyEffect) {
            moderatorOnly = true;
        } else if (packet instanceof ClientboundRemoveMobEffectPacket mePacket && mePacket.getEffect() instanceof ModeratorOnlyEffect) {
            moderatorOnly = true;
        }
